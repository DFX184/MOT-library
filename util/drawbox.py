...
def draw_box(image,pt1,pt2, label=None, color=None):
    if type(image) is not _np.ndarray:
        raise TypeError("'image' parameter must be a numpy.ndarray")
    try:
        left, top, right, bottom = int(pt1[0]), int(pt1[1]), int(pt2[0]), int(pt2[1])
    except ValueError:
    ...
    if label and not color:
        ...
        color = _COLOR_NAME_TO_RGB[_COLOR_NAMES[color_index]][0]

    if not color:
        color = _COLOR_NAME_TO_RGB[_DEFAULT_COLOR_NAME][0]
...
def draw_boxes(img,boxes,colors = None,labels = None):
